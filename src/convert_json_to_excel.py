...
OUTPUT_EXCEL_FILE = os.path.join(
    os.path.dirname(__file__), "..", "data", "CPQ_Import_Final.xlsx"
)
...
def make_name(text):
    ...
    if not text:
        return f"Unknown - {USER_FULL_NAME}"
...
def process_data():
    ...
    try:
        ...
        print(
            f"❌ Error: File '{INPUT_JSON_FILE}' not found. Please check the filename."
        )
        return
    ...
    if rows:
        ...
        print(
            "4. IMPORTANT: Map 'Category System ID' to the CPQ field 'Parent Category System ID'."
        )
    else:
